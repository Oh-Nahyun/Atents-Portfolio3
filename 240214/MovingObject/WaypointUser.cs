...
public class WaypointUser : MonoBehaviour
{
    ...
    /// <summary>
    /// 현재 위치가 도착 지점에 근접했는지 확인해주는 프로퍼티 (true면 목표 웨이포인트에 도착, false면 도착 못함)
    /// </summary>
    bool IsArrived
    {
        get
        {
            return (target.position - transform.position).sqrMagnitude < 0.01f; ///// 남은 거리가 0.1 이하이면 도달했다고 처리. true.
        }
    }
    ...
    protected virtual void OnMove()
    {
        ...
        if (IsArrived) // true면 웨이포인트 지점에 도착함
        {
            OnArrived();
        }
    }
